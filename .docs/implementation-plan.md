...
The Bonuses route now has a responsive promotional hero, recurring drop countdown, and Bear Hug, Honey Pot, and Lucky Paw offer cards using the supplied BearBet artwork. Activation actions remain honestly disabled until they are connected to persisted data. Expose available bonus definitions, the current award, and activation through authenticated server functions. Build the active state around the persisted award snapshot. Show required, completed, and remaining wagering, percentage, expiry, status, and eligibility failures.
