...
The visual base is near-black charcoal, raised graphite surfaces, warm honey and amber for primary actions, restrained cream text, and clear red, green, and blue semantic colors. Dark mode is the default product presentation. The app shell uses shadcn's sidebar behavior with the fixed navigation proportions, inset content frame, section rhythm, and mobile drawer pattern drawn from the Catalyst reference. The wordmark pairs the supplied bear mark with the self-hosted River Adventurer face through Tailwind's `font-logo` token. Geist remains the temporary body and UI face until the broader typography specimen is reviewed.
