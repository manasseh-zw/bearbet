...
- [ ] **B04. Build the authenticated hybrid sandbox bridge.** Link a BigBang provider player to the signed-in BearBet player, store the launch balance snapshot, fetch the final balance on explicit session close, and apply only the idempotent net delta through a labelled sandbox reconciliation operation. Do not mutate wallets from the current public throwaway launcher or infer per-round activity without callbacks.
