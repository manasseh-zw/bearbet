...
- [x] **P09. Build unified player history.** Combine ledger activity and gameplay activity in one route with views or filters for transactions, bets, wins, refunds, wallet bucket, operation type, period, and sort order. Keep the immutable ledger and gameplay records as the sources of truth; advanced date filtering remains deferred.
