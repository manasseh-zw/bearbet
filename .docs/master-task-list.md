...
- [x] **A06. Build operations management.** Inspect wallet and gameplay operations and approve or reject simulated withdrawals. The responsive withdrawal queue and read-only activity view selector use explicit projections, URL-backed single- and multi-select filters, offset queue pagination, and cursor activity pagination on top of the audited review mutation.
